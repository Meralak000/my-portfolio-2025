...
const ThemeToggle = () => {
  ...
  const toggle = () => {
    ...
    if (!button || !sync) return;
    
    const isPressed = button.matches("[aria-pressed=true]");
    if (sync.checked) {
      document.body.setAttribute("data-dark-mode", (!isPressed).toString());
    }
  ...
  return (
    <>
      <div className="controls">
        ...
        <input id="sync" type="checkbox" ref={syncRef} defaultChecked/>
      </div>
      <button className="toggle" aria-pressed="false" title="Toggle Dark Mode" ref={buttonRef}>
